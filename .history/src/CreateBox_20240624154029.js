...
const CreateBox = () => {
  ...
  const [amount, setAmount] = useState(0); // State để lưu số tiền muốn gửi kèm
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Tạo gift box và gửi kèm số tiền ETH
      await giftBox.methods.createBox(receiver, message, `ipfs://${ipfsHash}`).send({ from: currentAccount, value: web3.utils.toWei(amount.toString(), 'ether') });
      ...
      {!currentAccount ? (
        ...
        <React.Fragment>
          <form onSubmit={handleSubmit}>
            ...
            <div className="field">
              <label className="label">Amount of ETH to Send</label>
              <div className="control">
                <input
                  className="input"
                  type="number"
          ...
          <Modal isOpen={modalIsOpen} onRequestClose={() => setModalIsOpen(false)}>
            ...
            {boxLink && (
              <div>
                <QRCode value={boxLink} />
                <p><a href={boxLink}>{boxLink}</a></p>
              </div>
            )}
            <button onClick={() => setModalIsOpen(false)} className="button is-primary">Close</button>
          </Modal>
          <div>
            <h3 className="title is-4">Total Boxes Created by You: {boxes.length}</h3>
