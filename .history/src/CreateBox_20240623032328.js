  ...
  useEffect(() => {
    const loadBoxData = async () => {
      ...
      const transformedBoxes = boxes.map(box => {
        if (box.assetUrl.startsWith('ipfs://')) {
          box.assetUrl = `https://gateway.pinata.cloud/ipfs/${box.assetUrl.split('ipfs://')[1]}`;
        }
        return box;
      });
      setBoxes(boxes);
        ...
        <ul>
          {boxes.map((box, index) => (
            <li key={index}>
              ...
              <p><strong>Asset:</strong> {box.assetUrl}</p>
